...
public:
    ...
    int LCSHelper(int i , int j,string &text1, string &text2,vector<vector<int>> &dp){
        ...
        //if we already calculate it 
        if(dp[i][j] != -1){
            return dp[i][j];
        }
        //if the last index match 
        if(text1[i] == text2[j]){
            return dp[i][j] = 1 + LCSHelper(i - 1, j -1 , text1, text2, dp);
        }else{//if they not match then shrink both and pass the max ans
    ...
    int longestCommonSubsequence(string text1, string text2) {
        //this are the last index of the both the string
        int i = text1.size() - 1;
        int j = text2.size() - 1;

        //this is the dp for memoization
